...
- Delivery checks send immediately through the existing atomic queue, restricted to the authenticated user’s newly created test ID, with throttling. They must not drain other reminders or mark automatic delivery healthy. Keep their payloads separate from normal event plans. A test can expire without a running dispatcher; derive that state on read. Manual dispatcher success does not establish scheduled execution or receipt on a phone.
