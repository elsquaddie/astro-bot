...
Рабочая первая версия мобильного интерфейса AstroBot. React + TypeScript + Vite. Запускается отдельно от Python API и базы данных.
...
```bash
npm ci
npm run dev -- --host 127.0.0.1 --port 8875 --strictPort
npm test
npm run build
npm run check:runtime
```

Открыть http://127.0.0.1:8875/. Локальный сервер запускается на loopback. Геолокация в обычном размещении требует HTTPS; localhost разрешён браузерами как защищённый контекст. `npm run build` создаёт статические файлы в `dist/client/`; публикация в этой задаче не выполнялась.

На компьютере интерфейс показан в мобильном окружении с переключателем iPhone / Pixel 10. Это интерактивное приложение с реальными расчётами, но пока не отдельное нативное приложение и не установленная PWA. Правила окружения и защищённые файлы — в `AGENTS.md`.
...
- Сохранение планов на этом устройстве. Экспорт `.ics` с UTC-временем и `VALARM` за сутки, час, 15 минут или в момент наблюдения. Прошедшие варианты недоступны.
- Состояния первого запуска, расчёта, ошибок, пустого каталога и пустого сохранённого списка.

Само сохранение не отправляет уведомления. Пользователь должен импортировать `.ics` в календарь и проверить напоминание. Загрузка проверена в Chrome. Встроенный браузер Codex не завершил скачивание Blob-файла; после нажатия приложение подсказывает открыть обычный браузер. Telegram не подключён к этому клиенту.
